    ...
    let addTailLines = function(lines){
        ...
        for(let i = 0; i < lines.length; i++){
            for (let j = 0; j < lines[i].length; j++) {
                gameData[gameData.length - lines.length + i][j] = 3; // 修改：標記為干擾行
            }
        }
...
let swapHoldRemote = function (data) {
    ...
    let calculateGhostPosition = function () {
        let ghostPos = { x: cur.origin.x, y: cur.origin.y }; // 從當前方塊位置開始計算
    ...
    let updateGame = function () {
        refreshDiv(gameData, gameDivs); // 刷新當前方塊
        clearGhostPiece(); // 清除之前的幽靈方塊
        let ghostPos = calculateGhostPosition(); // 計算幽靈方塊位置
        renderGhostPiece(ghostPos); // 渲染幽靈方塊
    };
